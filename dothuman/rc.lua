...
mytextclock = awful.widget.textclock(markup("#7788af", "%x ") .. markup("#7788af", " %I:%M %p"))
...
-- WIFI
wifiicon = wibox.widget.imagebox(beautiful.widget_net)
wifiwidget = wibox.widget.textbox()
vicious.register(wifiwidget, vicious.widgets.wifi, 
    function(wifiwidget, args)
        link = args["{link}"]
        ssid = args["{ssid}"]
        if string.len(ssid) > 25 then
            ssid = string.sub(ssid, 1, 25) .. "-.."
        end
        return string.format("%s %s", ssid, link) .. "%"
    end,
    30, "wlp3s0")
...
for s = 1, screen.count() do
    ...
    mybottomwibox[s] = awful.wibox({ position = "bottom", screen = s, border_width = 0, height = 12 })
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey },            "d",
        function ()
            awful.util.spawn("dmenu_run -i -p 'Start:' -nb '" .. 
            beautiful.bg_normal .. "' -nf '" .. beautiful.fg_normal .. 
            "' -sb '" .. beautiful.bg_focus .. 
            "' -sf '" .. beautiful.fg_focus .. "'") 
        end),

    -- Shutdown, reboot, logout script
    awful.key({ modkey, "Control" },            "l",
        function ()
            os.execute(".scripts/powermenu.sh")
        end),
